...
public class SelectIntoTable extends JFrame {

	private static final long serialVersionUID = 1L;
	private Connection con;
	private Statement st;

	public SelectIntoTable() throws SQLException {
		...
		} catch (Exception e) {// demaisexceções
			e.printStackTrace();
		...
		try {
			...
			ResultSet resultSet = st.executeQuery("SELECT * FROM tabela1;");// processaresultadosdaconsulta
			ResultSetMetaData metaData = resultSet.getMetaData();
			int numberOfColumns = metaData.getColumnCount();
			System.out.println("Relação de pessoas cadastradas");
			for (int i = 1; i <= numberOfColumns; i++) {
				// exibeo nomedoscamposdatabela
	...
	public static void main(String args[]) throws SQLException {
		SelectIntoTable ex = new SelectIntoTable();
